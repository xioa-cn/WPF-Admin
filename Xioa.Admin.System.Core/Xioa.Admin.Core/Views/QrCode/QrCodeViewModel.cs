...
using Xioa.Admin.Core.Views.QrCode.Utils;
...
public partial class QrCodeViewModel : Xioa.Admin.Core.Services.ViewModels.ViewModelBase
{
    ...
    private async Task ReaderCode(Page page)
    {
        ...
        window.Hide();
        var bytes = Xioa.Admin.Core.Views.QrCode.Utils.QrCodeHelper.CaptureScreen(window);
    ...
    {
        var result = Xioa.Admin.Core.Views.QrCode.Utils.QrCodeHelper.ParseBarcode(bytes);
        await AddScanResultAsync(result);
    }

    private async Task AddScanResultAsync(string? result)
    {
        if (Xioa.Admin.Core.Views.QrCode.Utils.QrCodeHelper.IsNullOrEmpty(result))
        {
